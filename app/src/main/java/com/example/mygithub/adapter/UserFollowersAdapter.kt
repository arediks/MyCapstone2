...
class UserFollowersAdapter : ListAdapter<GitUsers, UserFollowersAdapter.MyViewHolder>(
    DIFF_CALLBACK
) {
    class MyViewHolder(private var binding: RecyclerviewDetailUserFollowBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(followers: GitUsers) {
            Glide.with(itemView.context).load(followers.avatarUrl).into(binding.ivDetailUserFollow)
